...
```json
{
  ...
  "breakout_daily": {"breakout": false, "recent_high": 0.6319, "pct_to_high": -14.62},
  "levels": {
    "current": 0.5395,
    "ma20w": 0.5855,
    "ma60w": 0.6961,
...
```json
{
  ...
  "levels": [
    ...
    {"key": "trailing", "icon": "🛑", "label": "移动止盈", "status": "若持有·触发即撤",
     "price": 0.7457, "cut": "清仓", "reason": "从阶段高点 0.8286 回撤 10%（至 0.7457）触发..."},
...
```
> `status` 取值：建议 / 可轻仓试探 / 暂不建议 / 不建议 / 若持有·触发即撤。每位都带 `reason`（引用具体净值/均线数字）。

## agent_analysis.json（🧠 agent 介入写，stage2 读）

```json
{
  "agent_reviewed": true,
  "commentary": "必填。≥30 字定性研判，引用 rightside.json 具体数字。例：'161725 距 2021 高点 -69%，当前 0.5395 跌破所有周线，空头排列。这不是右侧机会而是飞刀——白酒板块若未现主升浪右侧信号，纪律上零仓位。'",
  "sector_news": {
    "sector": "白酒",
    "as_of": "2026-07-25",
    "bullish": ["茅台 2026 年两次提价", "CR5 集中度 >55%", "五粮液股息率 7%+低估值"],
    "bearish": ["批价跌回 2017 水平", "三季报疲软、库存出清未完"],
    "summary": "筑底→复苏过渡期；消息面利好但技术面下降，等右侧确认，不因利好抄底"
  },
...
> stage2 把 `commentary` 追加到作战卡末尾的"🧠 Agent 定性研判"章节；
> `sector_news` 渲染成"📰 板块消息面"区块（利好/利空双栏 + 总结）。
