# fund-deep-analysis · 基金右侧交易决策 skill

> 一个 [Claude Code Agent Skill](https://agentskills.io/)。以**十年养基老手 + 右侧交易者**视角，分析国内场外基金，结合基金净值趋势 + 板块（共振 + 消息面），输出**右侧作战卡**：建仓/加仓/止盈/止损的具体净值位 + 理由 + 仓位。

![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)
![Python 3.9+](https://img.shields.io/badge/Python-3.9+-blue.svg)
![Agent Skills](https://img.shields.io/badge/Agent%20Skills-Standard-purple.svg)

## 这是什么

不是通用基金分析工具，而是**右侧交易决策工具**。整个分析只回答一个问题：

> **这只基金现在该不该动手、怎么动手。**

基于 [akshare](https://github.com/akfamily/akshare) 数据，周线为主、日线辅助的中长线趋势跟踪。

## 核心能力

| 能力 | 说明 |
|---|---|
| 🎯 右侧作战卡 | 建仓/加仓/止盈①②/移动止盈/止损，每个带 **status（建议/不建议/若持有触发）+ 理由**（引用净值/均线/板块数字） |
| 📊 板块共振 | 行业板块 + 宽基(14类) + 均衡三识别；基金 vs 板块/大盘双 regime → 🟢🟡🔴⚪ |
| 📰 板块消息面 | agent 用 WebSearch 搜主板块最新利好/利空，分类总结（深度模式） |
| 📈 风险指标 | 最大回撤 / 年化波动 / 夏普 / 卡玛 / 索提诺 + 多周期收益 |
| 🌐 HTML 报告 | Bloomberg 深色风，净值曲线 SVG + hover、净值位卡片、消息面双栏，单文件可分享 |
| 🔬 历史回测 | 右侧信号在该基金的历史胜率 / 收益（防盲信未验证信号，如白酒胜率仅 14%） |
| 💰 估值锚 | 宽基 PE 历史分位（防技术面高位接盘；行业基金数据源受限降级） |
| 🧩 组合体检 | 多基金相关性 + 行业暴露重叠（防单只都对、组合集中爆雷） |

## 效果示例（161725 招商中证白酒 · 2026-07-25）

```
当前净值: 0.5395   趋势: 下降（空头排列）   ⚠️ 深度回撤

▶ 操作建议: 观望 · 不进场        建议仓位: 0%
> 周线下降 + 空头排列，白酒板块同步下降🔴共振走弱。纪律不抄底。

🎯 关键净值位（含理由）
建仓    不建议      —       净值 0.5395 跌破 MA20(0.5855)/MA60(0.6961)，右侧未确认
止盈①   若持有触发  0.5233  跌破近端支撑，先减 30% 锁利润
移动止盈 若持有触发  0.7457  从阶段高点 0.8286 回撤 10% 触发
止损    若持有触发  0.5032  跌破前低，下跌结构延续，坚决清仓

📰 板块消息面（白酒）
🔼 利好: 茅台 2026 年内两次提价 · CR5 集中度 >55% · 五粮液股息 7%+低估值
🔽 利空: 批价跌回 2017 水平 · 三季报疲软 · 库存出清未完
💡 消息面「复苏早期」vs 技术面「下降」= 矛盾，纪律上零仓位，等右侧确认
```

## 快速开始

### 1. 安装

```bash
git clone https://github.com/qsw920204726/fund-deep-analysis.git
# 放到 Claude Code skills 目录：
#   Mac/Linux: ~/.claude/skills/fund-deep-analysis
#   Windows:   C:\Users\<你>\.claude\skills\fund-deep-analysis

cd fund-deep-analysis/scripts
pip install -r requirements.txt        # Windows 用 py -m pip
```

### 2. 用法（三选一）

**① 自然语言**（新开会话，skill 自动触发）
```
分析基金 161725
这只白酒基金现在能加仓吗
```

**② slash 命令**（先复制 commands/ 到用户级目录）
```bash
cp commands/*.md ~/.claude/commands/        # Mac/Linux
copy commands\*.md %USERPROFILE%\.claude\commands\   # Windows
```
然后：
```
/fund-analyze 161725        # 深度（agent 搜板块消息面 + 研判）
/fund-quick-scan 161725     # 30 秒快速（纯脚本，无消息面）
```

**③ 直接跑脚本**（跑完自动打开 HTML，加 `--no-open` 跳过）
```bash
cd scripts
py run.py 161725 --quick         # 快速出作战卡 + HTML（自动打开）
py run.py 161725                 # 深度：--stage1 → agent 介入 → --stage2
py lib/portfolio.py 161725 005827 110023   # 组合体检（多基金相关性 + 集中度）
```

## 工作原理

**两段式架构**（数据靠脚本，判断靠 agent）：

```
stage1 脚本:  净值 → 风险指标 → 周线趋势 → 板块共振 → 骨架决策
                                  ↓
agent 介入:   读骨架 → WebSearch 搜板块消息面 → 写定性研判
                                  ↓
stage2 脚本:  合并 agent 分析 → 右侧作战卡 + HTML 报告
```

**板块分析三层**：
- 🔧 技术面：基金 regime + 板块/大盘 regime → 共振信号灯
- 📰 消息面：agent 搜主板块最新利好/利空
- 🧠 研判：消息面 vs 技术面矛盾分析

## 右侧交易铁律

1. **不抄底、不猜顶** —— 趋势确认才动手
2. **宁可买贵 5%，不接飞刀**
3. **分批进、分批出**
4. **止损要快，止盈用移动止盈让利润奔跑**

> 关键纪律：**消息面利好 ≠ 买入**，必须和技术面右侧信号结合（消息面利好但技术面下降 = 飞刀，仍不抄底）。

## 目录结构

```
fund-deep-analysis/
├── SKILL.md                  # 入口（符合 Agent Skills 规范）
├── CLAUDE.md                 # Claude Code 上下文
├── commands/                 # slash 命令（fund-analyze / fund-quick-scan）
├── scripts/
│   ├── run.py                # 两段式入口（--quick / --stage1 / --stage2）
│   ├── requirements.txt
│   └── lib/                  # 7 个核心模块
│       ├── data_fetcher.py        # akshare 封装
│       ├── risk_metrics.py        # 回撤/波动/夏普/卡玛
│       ├── trend_signals.py       # 周线 20/60 + 日线趋势
│       ├── right_side_engine.py   # ★ 右侧决策引擎（灵魂）
│       ├── sector_exposure.py     # 板块识别（行业/宽基/均衡）
│       ├── sector_resonance.py    # 板块共振信号灯
│       ├── report_render.py       # Bloomberg HTML 报告
│       ├── backtest.py            # 历史回测（信号胜率）
│       ├── valuation.py           # 估值锚（PE 分位）
│       └── portfolio.py           # 组合体检（相关性 / 集中度）
├── references/               # 按需加载的方法论
├── assets/                   # 参数定值 + 数据契约 + 免责
└── reports/161725_*/         # 示例报告（markdown + HTML）
```

## 已验证

7 只基金 × 3 种 regime（下降/震荡/上升）× 全类型（行业/宽基/均衡）—— 板块识别、共振信号灯、消息面搜集、操作建议全部合理。

## 路线图

- ✅ 右侧作战卡（建仓/加仓/止盈/止损 + 理由 + 仓位）
- ✅ 板块共振（行业 + 宽基 14 类 + 均衡）
- ✅ 板块消息面（agent WebSearch）
- ✅ Bloomberg HTML 报告 + 宽基识别
- ✅ **历史回测**（信号胜率）+ **估值锚**（PE 分位）+ **组合体检**（相关性/集中度）—— 补齐"敢照着做"的关键缺口
- ⏳ Phase 3：基金经理画像 / 多基金对比 / 按条件筛选 / self-review gate
- ⏳ v2：Brinson 业绩归因 / 场内 ETF / 累计净值 / 行业指数估值

## ⚠️ 免责

本 skill 输出的是**右侧交易策略框架的机械信号 + agent 研判，非投资建议**。场外基金 T+1、净值滞后，信号仅供决策参考，盈亏自负。**投资有风险，入市需谨慎。**

## License

[MIT](LICENSE)
